...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def postprocess(self, prediction_dict, true_image_shapes):
    ...
    if self._number_of_stages == 3:
      ...
      for k in non_tensor_predictions:
        tf.compat.v1.logging.info('Removing {0} from prediction_dict'.format(k))
  ...
  def _add_detection_features_output_node(self, detection_boxes,
    ...
    with tf.compat.v1.name_scope('SecondStageDetectionFeaturesExtract'):
      ...
      batch_size = tf.shape(input=detection_boxes)[0]
      max_detections = tf.shape(input=detection_boxes)[1]
      detection_features_pool = tf.reduce_mean(
          input_tensor=detection_features_unpooled, axis=[1, 2])
      reshaped_detection_features_pool = tf.reshape(
          detection_features_pool,
          [batch_size, max_detections, tf.shape(input=detection_features_pool)[-1]])
      ...
      reshaped_detection_features = tf.reshape(
          ...
          [batch_size, max_detections,
           tf.shape(input=detection_features_unpooled)[1],
           tf.shape(input=detection_features_unpooled)[2],
           tf.shape(input=detection_features_unpooled)[3]])
    ...
    if groundtruth_masks_list is not None and self._resize_masks:
      ...
      for mask in groundtruth_masks_list:
        ...
        _, resized_mask, _ = self._image_resizer_fn(
            ...
            # `image_resizer_fn`.
            image=tf.zeros(tf.stack([tf.shape(input=mask)[1],
                                     tf.shape(input=mask)[2], 1])),
            masks=mask)
  ...
  def _sample_box_classifier_minibatch_single_image(
    ...
    cls_weights = tf.reduce_mean(input_tensor=cls_weights, axis=-1)
  ...
  def _postprocess_box_classifier(self,
    ...
    if refined_decoded_boxes_batch.shape[2] > 1:
      class_ids = tf.expand_dims(
          tf.argmax(input=class_predictions_with_background_batch[:, :, 1:], axis=2,
                    output_type=tf.int32),
          axis=-1)
      raw_detection_boxes = tf.squeeze(
          tf.compat.v1.batch_gather(refined_decoded_boxes_batch, class_ids), axis=2)
    else:
  ...
  def _loss_rpn(self, rpn_box_encodings,
    ...
    with tf.compat.v1.name_scope('RPNLoss'):
      ...
      batch_cls_weights = tf.reduce_mean(input_tensor=batch_cls_weights, axis=2)
      ...
      normalizer = tf.maximum(
          tf.reduce_sum(input_tensor=batch_sampled_indices, axis=1), 1.0)
  ...
  def _loss_box_classifier(self,
    ...
    with tf.compat.v1.name_scope('BoxClassifierLoss'):
      ...
      second_stage_loc_loss = tf.reduce_sum(
          input_tensor=second_stage_loc_losses * tf.cast(paddings_indicator,
                                            dtype=tf.float32))
      second_stage_cls_loss = tf.reduce_sum(
          input_tensor=second_stage_cls_losses * tf.cast(paddings_indicator,
                                            dtype=tf.float32))
      ...
      if prediction_masks is not None:
        ...
        mask_losses = self._second_stage_mask_loss(
            ...
            losses_mask=losses_mask)
        total_mask_loss = tf.reduce_sum(input_tensor=mask_losses)
        normalizer = tf.maximum(
            tf.reduce_sum(input_tensor=mask_losses_weights * mask_height * mask_width), 1.0)
  ...
  def _get_refined_encodings_for_postitive_class(
    ...
    # predictions
    refined_box_encodings_with_background = tf.pad(tensor=refined_box_encodings,
